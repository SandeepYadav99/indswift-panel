...
const LocationCard = lazy(() => import("./components/LocationCard/LocationCard.view"));
const PendingOfferTable = lazy(() => import("./components/WarehouseTables/PendingOfferTable.component"));
const InterviewsTable = lazy(() => import("./components/WarehouseTables/InterviewsTable.component"));
const GenericSlider = lazy(() => import("../EmployeePanel/EmployeeDashboard/component/Members/GenricSlider"));
...
const NewDashboard = () => {
    ...
    const locationData = useMemo(() => {
        return tiles?.locationData?.map((val) => {
            return <Suspense fallback={<div></div>}><LocationCard data={val}/></Suspense>;
        });
    }, [tiles?.locationData]);
                    ...
                    {tiles?.rp_stats?.length > 0 && tiles?.rp_stats?.map((item) => (
                        <div className={styles.plainPaper222}>
                            <div className={styles.deptWrapp}>
                                <div className={styles.rpLink}
                                     onClick={() => changeRPRoute(item?.job_id)}>{item?.job_code}</div>
