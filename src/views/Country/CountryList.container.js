...
import {
    ...
    actionChangePageCountries,
    actionChangeStatusCountries,
    actionFilterCountries,
    actionResetFilterCountries,
    actionSetPageCountries,
    actionCreateCountries,
    actionUpdateCountries
} from '../../actions/CountriesInfo.action';
import {serviceFetchCountries} from "../../services/Countries.service";
import {serviceGetCustomList, serviceGetCustomListNew} from "../../services/Common.service";
...
class TourTypeList extends Component {
    ...
    _renderCreateForm () {
        if (CreateProvider == null) {
            // import CreateProvider from './Create.container';
            CreateProvider = require('./Country.container').default;
        }
    ...
    _handleCities(data) {
        this.props.history.push('/country/region/'+data.id);
    }
    ...
    render() {
        ...
        const tableStructure = [
            ...
            //     label: 'Currency',
            //     sortable: true,
            //     render: (temp, all) => <div title={all.currency}>{all.currency}({all.currency_shortcode})</div>,
            // },
            // {
            //     key: 'language',
            //     label: 'Language',
            //     sortable: true,
            //     render: (temp, all) => <div title={all.language} style={{textTransform:'capitalize'}}>{all.language}({all.language_shortcode})</div>,
            // },
            // {
            //     key: 'city',
            //     label: 'Address',
            ...
            },
            {
                ...
                render: (temp, all) => (<div><IconButton className={'tableActionBtn'} onClick={this._handleEdit.bind(this, all)}><EditIcon fontSize={'small'} /></IconButton>
                    <Button onClick={this._handleCities.bind(this, all)} color={'primary'}>Region</Button></div>),
            },
            ...
            <div>
                <PageBox>
                    <div className={styles.headerContainer}>
                        <span className={styles.title}>Countries List</span>
                ...
                <SidePanelComponent
                    ...
                    title={'Countries'} open={this.state.side_panel} side={'right'}>
...
function mapStateToProps(state) {
    return {
        data: state.countries.present,
        total_count: state.countries.all.length,
        currentPage: state.countries.currentPage,
        serverPage: state.countries.serverPage,
        sorting_data: state.countries.sorting_data,
        is_fetching: state.countries.is_fetching,
        query: state.countries.query,
        query_data: state.countries.query_data,
    };
}

function mapDispatchToProps(dispatch) {
    return bindActionCreators({
        actionFetchData: actionFetchCountries,
        actionSetPage: actionSetPageCountries,
        actionResetFilter: actionResetFilterCountries,
        actionSetFilter: actionFilterCountries,
        actionChangeStatus: actionChangeStatusCountries,
        actionCreate: actionCreateCountries,
        actionUpdate: actionUpdateCountries
    }, dispatch);
}
