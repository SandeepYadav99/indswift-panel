...
import {
  serviceSendCVShortlistReminder,
  serviceSendFeedbackReminder,
  serviceSendOlrReminder,
...
const InterviewHistory = ({ historyDetail, historyData }) => {
  ...
  const handleFeedbackReminder = useCallback(
    (candidateId, jobId) => {
      if (!isSubmitting) {
        setIsSubmitting(true);
  ...
  const handleOlrReminder = useCallback(
    (candidateId, jobId) => {
      if (!isSubmitting) {
        setIsSubmitting(true);
        console.log("olr")
        ...
        return (
          <div className={styles.interviewWrapper} key={`historyCard_${index}`}>
            ...
            <SummaryView
              ...
              handleSendReminder={() => {
                handleFeedbackReminder(item.candidate_id, item.job_id);
              }}
            />
            <SummaryView
              ...
              handleSendReminder={() => {
                handleOlrReminder(item.candidate_id, item.job_id);
              }}
            />
          </div>
        );
