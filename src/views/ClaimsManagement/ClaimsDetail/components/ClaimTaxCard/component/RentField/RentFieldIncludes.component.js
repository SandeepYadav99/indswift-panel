...
import RentFieldIncludeFields from "./RentFieldIncludeFields.component";

const TEMP_OBJ = {
  landlord_name: "",
  rent: "",
  landlord_address: "",
  house_rent_evidence: null,
  landlord_pan: "",
};

const RentFieldIncludeForm = (
  ...
  const validateData = (index, type) => {
    ...
    fields.forEach((val, index) => {
      ...
      if (val?.rent && Number(val?.rent) > 100000) {
        if (!val?.landlord_pan) {
          err["landlord_pan"] = true;
        }
      }
...
export default forwardRef(RentFieldIncludeForm);
