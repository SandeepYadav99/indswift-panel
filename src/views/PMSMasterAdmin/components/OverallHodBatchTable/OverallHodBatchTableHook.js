...
const totalShow = 10;
const useOverallHodBatchTable = ({ Renderdata, getPmsList }) => {
  ...
  useEffect(() => {
    if (Renderdata?.length > 0) {
      const filteredData = Renderdata?.filter(
        (item) => item?.overall_hod_batch?.is_created
      );
...
export default useOverallHodBatchTable;
