...
import {
  ...
  BookmarkBorder as BookmarkEmpty,
  ...
  _handleSubmit(tData) {
    ...
    if (data) {
      ...
      tData = {
        ...newObj,
        status: status,
        is_dashboard: tData?.is_dashboard ? tData?.is_dashboard : false,
      };
    }
    ...
    if (remote_images?.length > 0) {
      const DbImage = remote_images?.map((value) => {
        const matchingObject = data?.images?.find(
          (obj) => obj?.image === value
        );
        if (matchingObject) {
          return {
        ...
        imagesArr.push(
          <div className={styles.imgContainer}>
            <div className={styles.imgLikeBtn}>
              <ButtonBase
                onClick={this._handleThumbnail.bind(this, "REMOTE", tempIndex)}
              >
            ...
            <div className={styles.imgBtn}>
              <ButtonBase
                onClick={this._handleDeleteImage.bind(
                  this,
                  "REMOTE",
                  index,
                  tempIndex
      ...
      form_values.images.forEach((val, index) => {
        imagesArr.push(
          <div className={styles.imgContainer}>
            ...
            <img src={URL.createObjectURL(val)} className={styles.img} alt="" />
          </div>
        );
        tempIndex++;
      });
      ...
      <div>
        ...
        <form onSubmit={handleSubmit(this._handleSubmit)}>
          ...
          <div className={styles.bookmarkLabelWrapper}>
            ...
            <span>
              Bookmarked Images will show on Employees Dashboard Event Section
            </span>
          </div>
...
function mapStateToProps(state) {
  return {
    form_values: state.form.hr_utsav_items
      ? state.form.hr_utsav_items.values
      : null,
  };
}
