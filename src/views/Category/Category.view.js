...
const asyncValidate = (values, dispatch, props) => {
    return new Promise((resolve, reject) => {
        if (values.name) {
            const value = values.category_code;

            if (lastValue == value && isExists && false) {
                reject({name: 'Category Name already Taken'});
            } else {
                const tempQuery = {category_code: value, name: values.name};
                const {data} = props;
                if (data) {
                ...
                serviceCategoryCheck(tempQuery).then((data) => {
                    ...
                    if (!data.error) {
                        ...
                        else if(data.data.category_code) {
                            error['category_code'] = 'Category Code Already Exist';
                            isError = true;
                        }
                        if (isError) {
                            reject(error);
...
class Category extends Component {
    ...
    componentDidMount() {
        ...
        if (data) {
            requiredFields = ['name','description','category_code'];
    ...
    _handleSubmit(tData) {
        ...
        const {data} = this.props;
        if (data) {
            this.props.handleDataSave({ ...tData, id: data.id }, 'UPDATE')
        } else {
            ...
            <div>
                ...
                <form onSubmit={handleSubmit(this._handleSubmit)}>
                    <div className={'formFlex'} style={{ }}>
                        ...
                        <div className={'formGroup'}>
                            <Field fullWidth={true} name="category_code" component={renderOutlinedTextField}
                                   margin={'dense'}
                                   normalize={titleNormalize}
                                   label="Category Code"/>
                        </div>
