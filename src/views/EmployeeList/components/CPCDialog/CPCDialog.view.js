...
import useCPCDialogHook from "./CPCDialog.hook";
...
const CPCDialog = ({isOpen, handleToggle, handleCsvUpload}) => {
    const classes = useStyles();
    const { changeTextData, errorData, form, handleSubmit, onBlurHandler, removeError, resData, isSubmitted, isSubmitting, isVerified } = useCPCDialogHook({isOpen, handleCsvUpload, handleToggle});
                    ...
                    <TableBody>
                        {resData.map((row) => (
                            <TableRow key={row.name}>
                                ...
                                <TableCell align="right">{row?.lookup_err?.join(', ')}</TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
    ...
    return (
        <div>
            ...
            >
                ...
                <div className={'flex'}>
                <div className={styles.upperFlex}>Import Employee CPC CSV</div>
                    <ButtonBase classes={{root: classes.closeBtn}} onClick={handleToggle}>
                        <Close />
                    </ButtonBase>
                </div>
                {/*</DialogTitle>*/}
