  ...
  const tableStructure = useMemo(() => {
    return [
      ...
      },
      {
        ...
        label: "% increment",
        sortable: false,
        render: (temp, all, index) => (
          <div className={styles.noWrap}>
            {
              <PercentageField
                id={all?.id}
                finalPercentage={all?.final_percentage}
                percentage={all?.increment_percentage}
                handleInputChange={handleValueChange}
              />
            }
          </div>
  ...
  return (
    <div>
      ...
      <DialogIncComponent
        ...
        handleConfirm={() => {
          !isFreezing && freezeIncrementPlanner();
        }}
      />
      {!isFreezed && (
