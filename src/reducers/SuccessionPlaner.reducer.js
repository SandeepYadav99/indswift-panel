import {
    ...
    UPDATE_DATA,
    DELETE_ITEM,
    FETCHED_YEAR, FETCHED_NEXT_YEAR, FETCHED_NEXT_NEXT_YEAR 
...
function mapPresetPRequest(all, pageId) {
    return all.filter((val, index) => {
        if (index >= (((pageId + 1) * constants.PAGE_VALUE) - constants.PAGE_VALUE) && index < (((pageId + 1) * constants.PAGE_VALUE))) {
            return val;
        }
    });
}
...
export default function (state = JSON.parse(JSON.stringify(initialState)), action) {
    switch (action.type) {
        ...
        case FETCHED: {
            ...
            return {...state, all: newAll, data: tableData, is_fetching: false, source}; // { ...state , all: newAll, data: tableData, serverPage: 1, currentPage: 1 };
        }
