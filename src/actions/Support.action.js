...
import {serviceCreateSupport, serviceGetSupport, serviceUpdateSupport} from "../services/Support.service";
...
export const DELETE_ITEM = 'DELETE_SUPPORT';

export function actionFetchSupport(index = 1, sorting = {}, filter = {}, shouldReset=false) {
    const request = serviceGetSupport({ index, row: sorting.row, order: sorting.order, ...filter });
    return (dispatch) => {
        if (shouldReset) {
            dispatch({
                type: CHANGE_PAGE,
...
export function actionCreateSupport(data) {
    const request = serviceCreateSupport(data);
    return (dispatch) => {
        request.then((data) => {
            if (!data.error) {
...
export function actionUpdateSupport(data) {
    const request = serviceUpdateSupport(data);
    return (dispatch) => {
        request.then((data) => {
            if (!data.error) {
...
export function actionDeleteSupport(id) {
    // const request = serviceDeleteSupport({ id: id});
    // return (dispatch) => {
...
export function actionChangePageSupportRequests(page) {
    return (dispatch) => {
...
export function actionChangeStatusSupport(id, status) {
