...
const useCandidateInterviewTable = ({jobId, handleClose }) => {
    ...
    useEffect(() => {
        ...
        const shortlistedCandidates = candidates.filter((val) => [status.CV_SHORTLISTED, status.INTERVIEW_ALIGNED, status.SELECTED,status.SHORTLISTED].indexOf(val.status) >= 0);
